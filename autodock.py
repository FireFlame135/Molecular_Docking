...
Modified my Tyler Mitton --- 9/2/2024
...
def convert_smi2pdbqt(masterfile):
    ...
    if os.path.exists(f'{home_path}/groups/grp_MolecularDock'):
        obabel = f'{home_path}/groups/grp_MolecularDock/openbabel-2.3.1/build/bin/obabel'
...
def prep_ZINC_lig(filename, working_folder=None, filelimit=None, slurm_gen=False, email=None):
    ...
    '''
    if not os.path.exists(f'{home}/groups/grp_MolecularDock'):
...
def prep_input(RecpPDB=None, LigPDB=None):
    ...
    if not os.path.exists(f'{home}/groups/grp_MolecularDock'):
        raise SystemError('Please run FDA Ligand prep on the super computer')
    #/home/username/groups/grp_MolecularDock
...
def package(working_folder=None, tar_out=None, master=None, result_file=None, output_struc=None):
    ...
    outFolder=f'/home/{username}/groups/grp_MolecularDock/autodock_result' 
...
#Below statement prevents the execution of main() when the script is run in PyMOL
